...
def get_session_file_upload_id_for_llm(id:str) -> str | None:
    file_uploads_meta_session = get_session_file_uploads_meta()
    for file in file_uploads_meta_session or []:
        if file.get("id") == id:
            return file.get("upload_id")
    return None

def set_session_file_upload_id_for_llm(id: str, upload_id:str):
    file_uploads_meta_session = get_session_file_uploads_meta()
    for file in file_uploads_meta_session or []:
        if file.get("id") == id:
...
def llm_file_for_company_name(name:str) -> List[LLMFile]:
    file_uploads_meta_session = get_session_file_uploads_meta()
    results = []
    for file in file_uploads_meta_session or []:
        if file.get("company_name") == name:
            logger.info(f"found match for company {name} {file}")
            results.append(LLMFile(file_name=file["filename"],file=bytes(), id=file["id"]))
    return results
